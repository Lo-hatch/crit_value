...
class PlotIt(mcPlot):
    ...
    def plot_scatter(self, xx,yy,xlab,ylab,fitname, iplot, oneline=False, ifcolor=True, iffit = False,clcvar=None, clcpro = None, x_lim=None,y_lim=None, tit=None, fitDirec=False, GM_method=None):
        ...
        if iffit:
            ...
            if fitname=='logi':
                for i, (xx, yy) in enumerate(zip(X, Y)):
                    ...
                    txt1 = 'critical x when y = 0.05'
                    value = xslope2
                    line1,=ax1.plot(x_fit, y_fit, color=cmap(i),linestyle='--',label='fitting line')
                    line2=ax1.axvline(x=value, color=cmap(i), linestyle=':',label=txt1)
                    if r<0.01:
                        txt = '0.0'
                    else:
                        txt = f'{my_pf.numstrFormat(r,2)}'
                    ax1.text(0.97,0.9-i*0.1, fr'$E_{{sat}}$ level{i+1}: {my_pf.numstrFormat(value,2)}, R2={txt}, RMSE={my_pf.numstrFormat(rmse,2)}' ,verticalalignment='bottom', horizontalalignment='right',
            transform=ax1.transAxes,color=cmap(i))
            elif fitname=='tanh':
        ...
        #line1a=ax1.axhline(y=0.05, color=light_light_grey, linestyle='--')
...
if __name__ == '__main__':
 ...
 "SE-Nor" ,"CN-Cng" ,"CN-Dan"]
    ...
    varread=['GPP','GPP_epotcan3','TVeg','epotcan3','gsw_sl','gsw_epotcan3_sl','wb_soilmean','rwc_soilmean_recal','rwc_soilmean','psi_soilmean']
    suffobs = '_flux.nc'
    varsoil = {
    'wb_soilmean': r'$\theta$ (m$^{3}$ m$^{-3}$)', #'mean soil water content (m m$^{-1}$)',
...
# Extract the two 'xxxx' parts before the .nc extension
    ...
    for isite in sites:
        ...
        result = []
        light_light_grey = "#6E6E6CC8"  # Dark grey
                ...
                for ix, xlab in varsoil.items():
                    ...
                    for imethod in calc_methods:
                        ...
                        for iseg in [0,1,2,3]:
                            ...
                            X.append(xx)
                            Y.append(yy)
                        tit = f'{isite} {isn} {imethod}'
                        tit = ''
                        iplot,xcmx,xccmx,xccmn,r,rmse,dycmx,dyccmx,dyccmn = self.plot_scatter(X,Y,xlab,ylab,'logi',
                           iplot,oneline=False,ifcolor=False,iffit = True,clcvar=clcvar,clcpro=clcpro,tit =tit ,fitDirec=True,y_lim=(0,0.6),x_lim = x_lim1)
    ...
    plt.savefig(f"{fout}z00_NTDfit_segs_SIV2.png", dpi=300, bbox_inches="tight", transparent=True)
    if iplot>0:
