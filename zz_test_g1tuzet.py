...
def row_to_string(the_id):
    # Read CSV
    # Select the row where ID == the_id
    row = params_data[params_data["ID"] == the_id]
    ...
    # Drop the ID column
    row = row.drop(labels="ID")
    row_dict = row.to_dict()
    # Build "var:value" strings
    parts = [f"{col}:{my_pf.numstrFormat(row[col],2)}" for col in row.index]

    # Join into one string
    return row_dict,", ".join(parts)
def NTD_stat(Y):
    yy = np.concatenate(Y)
    return np.percentile(yy,90)
...
class PlotIt(mcPlot):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.nrow     = 1     # # of rows of subplots per figure
        ...
        self.dyabc    = 1.03  # % of (max-min) shift up from lower x-axis
                              # for a,b,c,... labels
        # legend
    ...
    def plot_scatter(self, xx,yy,xlab,ylab,fitname, iplot, oneline=False, ifcolor=True, iffit = False,clcvar=None, clcpro = None, x_lim=None,y_lim=None, tit=None, fitDirec=True, GM_method=None,ifsigma=False):
        ...
        #         txt =  f'{lab}: xslope0.1={my_pf.numstrFormat(value,3)}'
        #         #txt =  f'{lab}: Max Curvature Change={my_pf.numstrFormat(xccmx1,3)}'
        #         ax1.text(0.67,0.9-i*0.1,txt ,verticalalignment='bottom', horizontalalignment='left',
        # transform=ax1.transAxes, fontsize=7,color=cmap(i))
            xx = np.concatenate(X)
            yy = np.concatenate(Y)
            if fitname=='logi':
            ...
            # line2b=ax1.axhline(y = 0, color=dark_grey, linestyle='--')
            txt =  f'whole: xslope0.1={my_pf.numstrFormat(value,3)}'
            #txt =  f'whole: Crit={my_pf.numstrFormat(xccmx1,3)}'
            ax1.text(0.67,0.9-(i+1)*0.1,txt ,verticalalignment='bottom', horizontalalignment='left',
...
if __name__ == '__main__':
 ...
 "SE-Nor" ,"CN-Cng" ]
    ...
    if platform.node()=='zlhp':
        ...
        fout = '/mnt/d/project/hydraulic/test/plot/f05_test_LHS/'
        ffigout = '/mnt/d/project/hydraulic/test/plot/fig_final/v20251111/'
    ...
    outfile = f"{fout}zz_test_g1tuzet.xlsx"
    params_data = pd.read_csv(f'{fout}g1tuzet_sample.csv')
    snd = {
    ...
    varread = ['Rainf','GPP','TVeg','Qle','Qh','gsw_sl',
                ...
                'epotcan3',
                'gsw_epotcan3_sl','GPP_epotcan3','dva']
    suffobs = '_flux.nc'
    varsoil = {
...
# Extract the two 'xxxx' parts before the .nc extension
    ...
    plot_config = {
        ...
        # },
        # 'E_wb': {
        #     'ylab': 'potential transpiration',
        #     'ylim': None
        # },
        'A/Asat': {
            'ylab': 'ratio of GPP to saturated GPP',
            'ylim': None
        },
        # '(GPP_wb - GPP)/GPP_wb': {
        ...
        for imethod in calc_methods:
            ...
            for iseg in [3]:
                for ikey,imod in snd.items():
                    print(f'soil type: {ikey}')
                    for itest in [1,2]:#range(3,5,1):
                        row_dict,params_str = row_to_string(itest)
                        print(f'Test ID: {itest}')
                        fullnames = []
                        shortnames = []
                        for isite in sites:
                            fn = glob.glob(f'{fp1}{isite}/{varlab0}/{imod}/test{itest}/outputs/site_out_cable_*.nc')
                        ...
                        for ix, xlab in varsoil.items():
                            ...
                            for isn in shortnames:
                                ...
                                ds = getattr(self,isn)
                                tit = f'{isn}: test{itest}: {params_str}'
                        #         iplot = my_pf.plot_multi_var_self(self,isn,['TVeg','epotcan3'],[isn],'TS','',iplot,plt.cm.tab10.colors[:3],
                        # iftextfrac=False,prc=1,tit =tit,x_lim=['2003-01-01','2003-12-31'],y_lim=None)  
                                if isn == 'US-SRM':
                                    y_lim=[0,4e-5]
                                    y_lim2 = [0.26,0.32]
                                elif isn == 'FR-LBr':
                                    ...
                                    y_lim2 = [0.275,0.425]             
                        #         iplot = my_pf.plot_multi_var_self(self,isn,['TVeg','epotcan3'],[isn],'TS','',iplot,plt.cm.tab10.colors[:3],
                        # iftextfrac=False,prc=1,tit =tit,x_lim=['2003-06-01','2003-09-30'],y_lim=y_lim,ify2=True,var2='wb_soilmean',y_lim2=y_lim2)  
                                clcvar = ds['epotcan3']
                                xvar = ds[ix]
                                ylab = config['ylab']
                                ...
                                elif 'A/Asat' in ylabel:
                                    yvar = ds['GPP']/ds['GPP_epotcan3']
                            ...
                            x_lim1 = [0.28,0.44]    
                            if iseg<4:
                                tit = f'{ikey}: seg{iseg+1}: test{itest}'
                            else:
                                tit = f'{ikey}: {imethod}:whole: test{itest}'       
                            tit = '' 
                
                            iplot,xcmx,xccmx,xccmn,r,rmse,xcons1,xcons2,xcons3,xslope1,xslope2,xslope3,xcons3K = self.plot_scatter(X,Y,xlab,ylab,'logi',
                                iplot,oneline=False,ifcolor=False,iffit = True,clcvar=clcvar,clcpro=clcpro,tit =f'{tit}' ,y_lim = [0.6,1],x_lim = x_lim1,
                                ifsigma=True)
                            # NTD90 = NTD_stat(Y)
                            # entry = [ikey, itest, NTD90]
                            # entry.extend(row_dict.values())
                            # result.append(entry)
                            # result.append([ix,ikey,itest,ylabel,imethod,iseg,xcmx,xccmx,xccmn,r,rmse,xcons1,xcons2,xcons3,xslope1,xslope2,xslope3,xcons3K])
    ...
    # df = pd.DataFrame(result, columns=["Xname", "soiltype","ID","yvar","method",'segID',"MaxCur","MaxCurChange","MinCurChange","R2","RMSE","cons0p03","cons0p04","cons0p05",'slope0p05','slope0p1','slope0p15',"cons0p05Kernel"])
    # df = pd.DataFrame(result, columns=["soiltype","ID","NTD90",'g1tuzet',"psi_50_leaf", "kmax", "P50", "P88dP50"])
    ...
    #     plt.savefig(f"{ffigout}z00_NTDfit_multisiteV2.png", dpi=300, bbox_inches="tight", transparent=True)
    # elif iplot>3:
    if iplot>0:
