...
class PlotIt(mcPlot):
    ...
    def plot_scatter(self, xx,yy,xlab,ylab,fitname, iplot, oneline=False, ifcolor=True, iffit = False,clcvar=None, clcpro = None, x_lim=None,y_lim=None, tit=None, fitDirec=False, GM_method=None):
        ...
        if ifcolor:
            ...
            #             r, rmse, x_fit,y_fit,bk= my_af.linear_plateau_fitting(xx,yy)
            #             labtxt = f'{lab}: crit={my_pf.numstrFormat(xcmx,2)},bk={my_pf.numstrFormat(bk,2)}'
                       
            #             line1,=ax1.plot(x_fit, y_fit, color=cmap(i),linestyle='--',label='fitting line')
            #             line2=ax1.axvline(x=xcmx, color=cmap(i), linestyle=':')
            #             line2x=ax1.axvline(x=bk, color=cmap(i), linestyle='-')
            #             scatter_handles.append(sc)
            #             ax1.text(0.6,1-i*0.1, labtxt ,verticalalignment='bottom', horizontalalignment='left',transform=ax1.transAxes, fontsize=7,color=cmap(i))
            ...
            labtxt = f'whole: crit={my_pf.numstrFormat(xcmx,2)},bk={my_pf.numstrFormat(bk,2)},bk2={my_pf.numstrFormat(xcmx_lp1,2)}'
            
            line1,=ax1.plot(x_fit, y_fit, color='k',linestyle='--',label='fitting line')
            line2=ax1.axvline(x=xcmx, color='k', linestyle=':')
            line2x=ax1.axvline(x=bk, color='k', linestyle='-')
            #line5a,=ax1.plot(x_fit_lp1, y_fit_lp1, color='g',linestyle='-',label='plateau-linear-plateau)')
            scatter_handles.append(sc)
            ax1.text(0.6,1-(i+1)*0.1, labtxt ,verticalalignment='bottom', horizontalalignment='left',transform=ax1.transAxes, fontsize=7,color=cmap(i))
    ...
    for ylabel, config in plot_config.items():
        ...
        # print(f'   shortname {shortnames}')
                        ...
                        for ix,xlab in varsoil.items():
                            ...
                            iplot,xcmx,xccmx,xccmn,bk,bk2,r,rmse,r_lp,rmse_lp,r_lp1,rmse_lp1 = self.plot_scatter(X,Y,xlab,ylab,'linear-plateau',iplot,oneline=False,ifcolor=False,iffit = True,clcvar=clcvar,tit =tit ,fitDirec=True)  
                        
                            result.append([ix,ikey,itest,ylabel,imethod,iseg,xcmx,xccmx,xccmn,bk,bk2,r,rmse,r_lp,rmse_lp,r_lp1,rmse_lp1])
    df = pd.DataFrame(result, columns=["Xname", "soiltype","ID","yvar","method",'segID',"MaxCur","MaxCurChange","MinCurChange",'BreakPoint',
