...
class PlotIt(mcPlot):
    ...
    def plot_scatter(self, xx,yy,xlab,ylab,fitname, iplot, oneline=False, ifcolor=True, iffit = False,clcvar=None, clcpro = None, x_lim=None,y_lim=None, tit=None, fitDirec=False, GM_method=None,ifsigma=False):
        ...
        y_c,y_cc,slopes,xcmx,xccmx1,xccmn1,r,rmse,xcons1,xcons2,xcons3,xslope1,xslope2,xslope3,xcons3K = [np.nan] * 15
        ...
        if iffit:
            ...
            if fitname=='logi':
                ...
                r, rmse,x_fit,y_fit,y_3d,x_3d,y_c,xcmx,xcmn3,y_cc,xccmx3,xccmn3,xcons1,xcons2,xcons3,xslope1,xslope2,xslope3 = my_af.exponential_strench_decrease_fitting(xx,yy,fitDirec=fitDirec,GM_method=GM_method)
            elif fitname=='Dsig':
            ...
            xpos = 0.54
            ypos = 0.6
            # ax1.text(xpos,ypos-0.05, f'R2={txt},RMSE={my_pf.numstrFormat(rmse,2)}' ,verticalalignment='bottom', horizontalalignment='left',
            ...
            line2=ax1.axvline(x=xcmx, color=maxcurcl, linestyle='--',label='critical threshold (max curvature)')
            ...
            line4a=ax1.axvline(x=xcons3, color=conscl, linestyle='--',label='critical threshold (y = 0.05)')
            line4b=ax1.axvline(x=xslope2, color=dark_grey, linestyle=':',label='critical threshold (10% of max slope)')
            ...
            # ax1.text(xpos,ypos-0.05-0.07, f'constant={my_pf.numstrFormat(xcons3,2)}' ,verticalalignment='bottom', horizontalalignment='left',
            # transform=ax1.transAxes,color=conscl)
            # ax1.text(xpos,ypos-0.05-0.07, f'slope10%={my_pf.numstrFormat(xslope2,2)}' ,verticalalignment='bottom', horizontalalignment='left',
            # transform=ax1.transAxes,color=dark_grey)
            ax2 = ax1.twinx()
            line5,=ax2.plot(x_fit,slopes,color=dark_grey, linestyle='-',label='slope')
            plt.setp(ax2, ylabel='Slope')
            ax3 = ax1.twinx()
            # Move the third y-axis further to the right
            # #ax3.spines["right"].set_position(("axes", 1.15))  # shift to 15% right of the original axis
            if not np.isnan(y_c).all():
                ax3.spines['right'].set_position(('outward', 40)) 
                ax3.spines['right'].set_color(maxcurcl) # Offset the third axis
                line6,=ax3.plot(x_fit, y_c, color=maxcurcl, linestyle='-', label='curvature line')
                ax3.set_ylabel('curvature', color=maxcurcl)
                ax3.tick_params(axis='y', labelcolor=maxcurcl)
        ...
        if iplot==1:
            ...
            lines = [line1,line6,line5,line2,line4a,line4b]
    ...
    def hist_tmp(self,xx,yy,iplot):
        ...
        ax1.hist(xx, bins=100, edgecolor='black')
        if (iplot == nplotperpage):
        # save one pdf page, zihanlu
            self.plot_save(fig)
            iplot = 0
...
if __name__ == '__main__':
 ...
 "SE-Nor" ,"CN-Cng" ,"CN-Dan"]
    ...
    if platform.node()=='zlhp':
        ...
        ffigout = '/mnt/d/project/hydraulic/test/plot/fig_final/V20251217/'
    ...
    if drydown:
        segs = [4]
        outfile = f"{fout}result_drydown_BASEdc98fe39.xlsx"
        monthss = [1,2,3,12]
        monthnn = [6,7,8,9]
    else:
        segs = [0,1,2,3,4]
                    ...
                    for imethod in calc_methods:
                        for iseg in segs:
                            ...
                            if isite=='US-SRM':
                                ...
                                ifsigma = False
                            #iplot = self.hist_tmp(xx,yy,iplot)
                            iplot,xcmx,xccmx,xccmn,r,rmse,xcons1,xcons2,xcons3,xslope1,xslope2,xslope3,xcons3K = self.plot_scatter(xx,yy,xlab,ylab,'logi',iplot,oneline=False,ifcolor=False,iffit = True,clcvar=clcvar,clcpro=clcpro,tit =tit ,fitDirec=True,ifsigma = ifsigma)
                   
                            result.append([ix,isn,ylabel,imethod,iseg,xcmx,xccmx,xccmn,r,rmse,xcons1,xcons2,xcons3,xslope1,xslope2,xslope3,xcons3K])
        if writeout:
            ...
            if not os.path.exists(outfile):
                ...
                with pd.ExcelWriter(outfile, mode="w", engine="openpyxl") as writer:
                    df.to_excel(writer, sheet_name=f"{isite}_{tag}", index=False)
            else:
