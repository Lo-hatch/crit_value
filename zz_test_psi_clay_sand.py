                ...
                for ix in varsoil:
                    ...
                    for imethod in calc_methods:
                        for iseg in segs:
                         ...
                         iplot,oneline=False,ifcolor=False,iffit = True,clcvar=clcvar,clcpro=clcpro,tit =tit ,fitDirec=True,ifsigma = ifsigma)
                   

              
                
        # iplot = my_pf.plot_scatter_simple(X[0], X[1],shortnames[0],shortnames[1],oneline=True)
        # plt.savefig(f"{fout}LBr_psicheck1.png", dpi=300, bbox_inches="tight", transparent=True)
        # iplot = my_pf.plot_scatter_simple(X[0], X[2],shortnames[0],shortnames[2],oneline=True)
        # plt.savefig(f"{fout}LBr_psicheck2.png", dpi=300, bbox_inches="tight", transparent=True)
        # iplot = my_pf.plot_scatter_simple(X[1], X[2],shortnames[1],shortnames[2],oneline=True)
        # plt.savefig(f"{fout}LBr_psicheck3.png", dpi=300, bbox_inches="tight", transparent=True)
