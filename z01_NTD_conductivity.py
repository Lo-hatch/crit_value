...
class PlotIt(mcPlot):
    ...
    def plot_scatter(self, xx,yy,xlab,ylab,fitname, iplot, oneline=False, ifcolor=True, iffit = False,clcvar=None, clcpro = None, x_lim=None,y_lim=None, tit=None, fitDirec=False, GM_method=None,ifsigma=False):
        ...
        if iffit:
            ...
            # best = xccmx1
            # y2 = y_cc
            ax1.axvline(x=xmark, color='y', linestyle='-')
            ...
            line4a=ax1.axvline(x=xcons3, color='c', linestyle='--',label='constant0.05')
            line4b=ax1.axvline(x=xslope2, color='c', linestyle=':',label='10pslope')
            ...
            transform=ax1.transAxes,color=clc, fontsize=6)
            ax1.text(0.02+0*0.16+0.02,0.9-4*0.1, f'constant={my_pf.numstrFormat(xcons3,2)}' ,verticalalignment='bottom', horizontalalignment='left',
            transform=ax1.transAxes,color=clc, fontsize=6)
            ax1.text(0.02+0*0.16+0.02,0.9-5*0.1, f'10pslope={my_pf.numstrFormat(xslope2,2)}' ,verticalalignment='bottom', horizontalalignment='left',
            transform=ax1.transAxes,color=clc, fontsize=6)
            ax2 = ax1.twinx()
        ...
        if iplot==1:
            ...
            ll = ax1.legend(lines,labels,frameon=self.frameon, ncol=4,
...
if __name__ == '__main__':
 ...
 "SE-Nor" ,"CN-Cng" ,"CN-Dan"]
    ...
    varread = ['Rainf','GPP','TVeg','Qle','Qh','gsw_sl',
                ...
                'epotcan3',
                'gsw_epotcan3_sl','GPP_epotcan3','ksoilmean','krootmean','kplant','kbelowmean',
                'psi_can_sl']
    suffobs = '_flux.nc'
    varsoil = {
    ...
    for isite in sites:
        ...
        for isn in shortnames:
           ...
           # gs_epotvpd = ds['gsw_epotvpd_sl']
            gs = ds['gsw_sl']
            Rplant = 1/ds['kplant']
            Rsoil = 1/ds['ksoilmean']
            Rroot = 1/ds['krootmean']
            Rbelow = 1/ds['kbelowmean']
            Rtotal = Rplant + Rsoil + Rroot
            ...
            plot_config = {
                ...
                'Relative Rsoil': {
                    'var': Rsoil/(Rroot+Rplant),
                    'ylab': r'$\frac{R_{soil}}{R_{total}}$',
                    'ylim': (-0.55, 1.5)
                },
                'psileaf': {
                    'var': ds['psi_can_sl'],
                ...
                # 'Relative Rroot': {
                #     'var': Rroot/Rtotal,
                #     'ylab': r'$\frac{R_{root}}{R_{total}}$',
                #     'ylim': (-0.55, 1.5)
                # },
                # 'Relative Rplant': {
                #     'var': Rplant/Rtotal,
                #     'ylab': r'$\frac{R_{plant}}{R_{total}}$',
                #     'ylim': (-0.55, 1.5)
                # },
                # 'log10 Rsoil': {
                #     'var': np.log10(Rsoil),
                #     'ylab': r'$log10(R_{soil})$',
                #     'ylim': (-0.55, 1.5)
                # },
                # 'Rsoil': {
                #     'var': (Rsoil),
                ...
                # 'log10 Rroot': {
                #     'var': np.log10(Rroot),
                #     'ylab': r'$log10(R_{root})$',
                #     'ylim': (-0.55, 1.5)
                # },
                # 'log10 Rplant': {
                #     'var': np.log10(Rplant),
                #     'ylab': r'$log10(R_{plant})$',
                #     'ylim': (-0.55, 1.5)
                # },
            }
                ...
                for ix in varsoil:
                    ...
                    for imethod in calc_methods:
                        for iseg in segs:
                            ...
                            iplot,xcmx,xccmx,xccmn,r,rmse,xcons1,xcons2,xcons3,xslope1,xslope2,xslope3,xcons3K = self.plot_scatter(xx,yy,ix,ylabel,'logi',iplot,oneline=False,ifcolor=False,iffit = True,clcvar=clcvar,clcpro=clcpro,tit =tit ,fitDirec=True,ifsigma = ifsigma)
                            if np.isnan(xmark):
                                xmark =  xslope2  
