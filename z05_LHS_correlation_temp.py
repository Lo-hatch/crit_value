...
if __name__ == '__main__':
    ...
    savetag = ''
    plotorstat = {
        'plotcorr':{'plot':True,'figname':f'z05_LHS_correlation_scatter_{savetag}'},
         'calcorr':{'plot':True,'figname':f'correlation_{savetag}'},
               }

    self = PlotIt('Scatter Plot')

    figout = '/mnt/d/project/hydraulic/test/plot/fig_final/ppt0129/'
    sites = ['UIFC']
    ...
    df1 = df1.rename(columns={indexname1: "NTD"})
    df_merged = df1.merge(
        df2,
        on=commonkeys,
        how="inner"
    ...
    # Filter to only plot for clay loam
    soil_types = ['fine clay']

    if plotorstat["plotcorr"]["plot"]:
        
        figname = plotorstat["plotcorr"]["figname"]
        # Removed conditional ncol override - using ncol = 2 as specified
        iplot=0
            ...
            for isoil in soil_types:
                for col, label in cols_label.items():
                    ...
                    if iplot % self.ncol == 0:
                        ...
                        ylab = ''
                    # Add x label for each subfigure
                    xlab = label
                    ...
                    # Add figure number label (a, b, c, etc.)
                    nplotperpage = self.nrow * self.ncol
                    ax1 = self.fig.axes[-1]  # Get the most recently added axes
                    # When iplot == 0 after plot_scatter_simple, it means we just completed a full page
                    # In that case, use nplotperpage for the plot position
                    # Also check if this is the last plot - if so, ifig will be incremented on next iteration
                    if iplot == 0:
                        # Just completed a page, use nplotperpage for this plot's position
                        plot_number = nplotperpage
                        # Use current ifig (will be incremented on next call)
                        current_ifig = self.ifig
                    else:
                        plot_number = iplot
                        current_ifig = self.ifig
                    
                    abc2plot(ax1, self.dxabc, self.dyabc,
                            (current_ifig - 1) * nplotperpage + plot_number,
    ...
    self.close()
            ...
            # 2.1 Filter df2 for this soil type + Xname condition
    # df2_filt = df2[(df2["Xname"] == xname) & 
    #             (df2["soiltype"] == isoil)]
    ...
    #         # 2. Merge on "ID"
    #         df = pd.merge(df1, df2_filt, on="ID")
    #         df = df[~df['ID'].isin([1,3,5,9,21])]
    #         # 3. Sort by cons0p05
    #         df_sorted = df.sort_values(by=indexname)

    #         # 4. Lowest 4 and largest 4
    #         lowest4 = df_sorted.head(4)
    #         largest4 = df_sorted.tail(4)
    ...
    #         # -------------------------------------------------------
    #         # 4. Loop over columns to plot each subplot
    #         # -------------------------------------------------------
    #         for ax, col in zip(axes, cols_to_plot):
    #             if plottype==1:
    #                 # KDE distribution
    #                 df_sorted[col].plot(kind="kde", linewidth=2, ax=ax)

    #                 # Overlay lowest4
    #                 ax.scatter(lowest4[col], [0]*len(lowest4),
    #                         s=60, marker='o', label="Lowest 4")

    #                 # Overlay highest4
    #                 ax.scatter(largest4[col], [0]*len(largest4),
    #                         s=60, marker='x', label="Top 4")
    #                 ax.set_xlabel(indexname)
    #                 ax.set_ylabel(f'{col}-density')
    #             elif plottype==2:

    #             elif plottype==3:
    #                 r, p = pearsonr(df_sorted[indexname], df_sorted[col])
    ...
    #             ax.set_title(f"{col} — Soil type {isoil}")
    #             ax.legend()
    #         if plottype in [1,2]:
    #             plt.tight_layout()
    #             out_path = os.path.join(out_dir, f"scatter_{isoil}{tag}.png")
    #             fig.savefig(out_path, dpi=300)
    #             plt.close(fig)   # close figure to free memory

    #             print(f"Saved plot for soil type {isoil} → {out_path}")
    #             plt.show()
    #         elif plottype == 3:
    #             corr_df = pd.DataFrame(correlation_results)
    #             if not os.path.exists(f'{out_dir}correlation.xlsx'):
    #                 with pd.ExcelWriter(f'{out_dir}correlation.xlsx', mode="w") as writer:
    #                     corr_df.to_excel(writer, sheet_name=f"correlation{tag}", index=False)
    #             else:
    #                 with pd.ExcelWriter(f'{out_dir}correlation.xlsx', mode="a", if_sheet_exists="replace") as writer:
    #                     corr_df.to_excel(writer, sheet_name=f"correlation{tag}", index=False)
