        ...
        for isn in shortnames:
           ...
           # gs_epotvpd = ds['gsw_epotvpd_sl']
            ...
            for ylabel, config in plot_config.items():
                ...
                keys = list(varsoil.keys())
                iplot = my_pf.plot_scatter_simple(X[0], X[1],keys[0],keys[1],oneline=True)
                plt.savefig(f"{fout}wbcheck1.png", dpi=300, bbox_inches="tight", transparent=True)
                iplot = my_pf.plot_scatter_simple(X[0], X[2],keys[0],keys[2],oneline=True)
                plt.savefig(f"{fout}wbcheck2.png", dpi=300, bbox_inches="tight", transparent=True)
                iplot = my_pf.plot_scatter_simple(X[1], X[2],keys[1],keys[2],oneline=True)
    plt.savefig(f"{fout}wbcheck3.png", dpi=300, bbox_inches="tight", transparent=True)
