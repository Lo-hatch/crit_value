...
class PlotIt(mcPlot):
    ...
    def plot_boxplot(self,df,iplot):
        ...
        if tag == 'NTD':
            ...
            plt.ylabel(r'$\frac{(E_{sat}-E)}{E_{sat}}$')
        else:
            plt.ylabel( r'normalized f$_{LE}$',)
...
# Extract the two 'xxxx' parts before the .nc extension
    ...
    for isite in sites:
                ...
                for ix in varsoil:
                    ...
                    for imethod in calc_methods:
                        for iseg in segs:
                            ...
                            df["x_bin"] = pd.cut(df["x"], bins=bins, labels=labels, include_lowest=True)
                            dfs[isn] = df
        ...
        iplot = self.plot_boxplot(df_all,iplot)
        plt.savefig(f"{fout}boxplot_psibin{tag}.png", dpi=300, bbox_inches="tight", transparent=True)
    # if iplot>0:
