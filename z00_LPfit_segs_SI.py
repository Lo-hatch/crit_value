            ...
            for i, (xx, yy) in enumerate(zip(X, Y)):
                if iffit:
                    if fitname=='linear-plateau':
                        ...
                        labtxt = f'Esat level{i+1}: breakpoint={my_pf.numstrFormat(xcmx,2)},R2={txt},RMSE={my_pf.numstrFormat(rmse,2)}'
                        sc = ax1.scatter(xx,yy, facecolors='none',edgecolors=cmap(i), s=9, label=labtxt)
                        line1,=ax1.plot(x_fit, y_fit, color=cmap(i),linestyle='--',label='fitting line')
                        line2=ax1.axvline(x=xcmx, color=cmap(i), linestyle=':',label='break point')
...
if __name__ == '__main__':
    ...
    sites=[ "AU-ASM",  "US-SRM" ,"AU-Gin" , "AU-How", "FR-Pue", "IT-Cp2" ,"ZM-Mon", "IT-Ro1" ,"IT-SRo",
 ...
 "SE-Nor" ,"CN-Cng" ,"CN-Dan"]
    ...
    if platform.node()=='zlhp':
        ...
        fout = '/mnt/d/project/hydraulic/test/plot/f03b_test_liear_plateau/'
    ...
    outfile = f"{fout}result_LP_newRNumtag.xlsx"
    tag = ''
    vars=['GPP','Qle']     
            ...
            for ylabel, config in plot_config.items():
                for ix,xlab in varsoil.items():
                    ...
                    for imethod in calc_methods:
                        ...
                        iplot = self.plot_scatter(X,Y,xlab,ylab,'linear-plateau',iplot,oneline=False,ifcolor=False,iffit = True,clcvar=clcvar,tit =tit ,fitDirec=True)
