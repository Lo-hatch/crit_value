...
class PlotIt(mcPlot):
    ...
    def plot_scatter(self, xx,yy,xlab,ylab,fitname, iplot, oneline=False, ifcolor=True, iffit = False,clcvar=None, clcpro = None, x_lim=None,y_lim=None, tit=None, fitDirec=False, GM_method=None):
        ...
        if iplot==1:
            ...
            ll = ax1.legend(lines,labels,frameon=self.frameon, ncol=3,
        ...
        # save one pdf page, zihanlu
            self.plot_save(fig)
            iplot = 0
        return iplot,xcmx,xccmx1,xccmn1,xcmx_lp,xcmx_lp1,r,rmse,r_lp,rmse_lp,r_lp1,rmse_lp1
                ...
                for ix in varsoil:
                    ...
                    for imethod in calc_methods:
                        for iseg in segs:
                            ...
                            min_val = np.nanmin(xx)
                            max_val = np.nanmax(xx)
                            iplot,xcmx,xccmx,xccmn,bk,bk1,r,rmse,r_lp,rmse_lp,r_lp1,rmse_lp1 = self.plot_scatter(xx,yy,ix,ylabel,'logi',iplot,oneline=False,ifcolor=False,iffit = True,clcvar=clcvar,tit =tit ,fitDirec=True,x_lim=[0.26,0.4])
                            result.append([ix,isn,ylabel,imethod,iseg,xcmx,xccmx,xccmn,bk,bk1,r,rmse,r_lp,rmse_lp,r_lp1,rmse_lp1])
    # Save to CSV
